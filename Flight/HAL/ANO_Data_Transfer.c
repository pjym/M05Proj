...
//			cnt = 0;	f.send_pid = 0;	
	...
	{		
		...
		switch(cnt)
		{
			case 1:	ANO_DT_Send_Status(-Angle.roll,Angle.pitch,-Angle.yaw,mini.flow_High,ALL_flag.slock_flag,ALL_flag.unlock);	 // 飞控状态
			break;
			case 2:	
			...
			break;
			case 3: ANO_DT_Send_RCData(Remote.thr,Remote.yaw,Remote.roll,Remote.pitch,Remote.AUX1,Remote.AUX2,Remote.AUX3,Remote.AUX4,NRF_SSI,Remote.AUX6);  // 遥控器通道数据
			break;
			case 4:	ANO_DT_Send_Power(voltage/10,Remote.AUX5,1,NRF_SSI,test_flag,set_flag);   //电压：飞机电压 / 电流：遥控器电压
			break;
			case 5:	 ANO_DT_Send_Senser2(FlightData.High.bara_height,0);  //气压计、超声波 高度数据
			break;
			case 6:	ANO_DT_Send_speed(pixel_flow.loc_x, pixel_flow.loc_y,FlightData.High.bara_height);  cnt = 0;		//光流模块数据					
			break;
